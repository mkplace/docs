...
Esta página cobre o login **de usuário** no navegador — diferente da
[autenticação de aplicação](/sellers/autenticacao), que usa `client_credentials`
(máquina-a-máquina). Aqui, quem coleta a senha é o **Keycloak**; seu aplicativo
nunca a vê e recebe apenas **tokens**.
...
| Cenário | Fluxo | Página |
|---|---|---|
| Servidor/integração chama a API sem usuário | `client_credentials` | [Autenticação](/sellers/autenticacao) |
| Usuário faz login no navegador / app | **Authorization Code + PKCE** | *esta página* |

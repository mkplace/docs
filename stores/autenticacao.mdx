...
<Note>
  **Nova forma de autenticação:** além da autenticação por JWT auto-assinado
  descrita nesta página, há agora o anexo
  **[Login de usuário via OIDC (Authorization Code + PKCE)](/stores/login-oidc)** —
  login interativo do usuário pelo IDP da Mkplace, com a sessão entregue por
  token/cookie (sem nada na querystring).
</Note>
